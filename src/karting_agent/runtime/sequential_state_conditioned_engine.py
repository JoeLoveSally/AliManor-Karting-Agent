"""State-conditioned runtime for v4-A explicit temporal features."""
...
class CachedSequentialSwitchModel(Protocol):
    def predict_switch(
        self,
        inputs: np.ndarray,
        ...
        *,
        ...
        """Return probability that the current state should be flipped."""


@dataclass(frozen=True)
class SequentialStateConditionedRuntimeConfig:
    target_fps: float
...
class SequentialStateConditionedRuntimeEngine:
    """Run v4-A and pass temporal frame ids to the feature-caching model runner."""

    def __init__(
        self,
        *,
        model: CachedSequentialSwitchModel,

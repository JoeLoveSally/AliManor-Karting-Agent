#!/usr/bin/env python3
"""Evaluate v4-A2 static switch quality and stateful offline replay."""
...
from evaluate_model_v4a import (  # noqa: E402
    StatefulDecision,
    binary_metrics,
    chatter_summary,
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Evaluate v4-A2 temporal MLP policy.")
    parser.add_argument(
...
def artifact_dir_from_config(raw: dict[str, object]) -> Path:
    ...
    return ROOT / "artifacts" / "models" / str(
        artifact.get("name", "mobilenet_v3_small_v4a2")
    )
...
def main() -> int:
    ...
    output_path = (
        args.output.resolve()
        if args.output
        else artifact_dir / "evaluation" / f"{args.split}_v4a2.json"
    )

    metadata = json.loads(metadata_path.read_text(encoding="utf-8"))
    if metadata.get("model_family") != "temporal_mlp_state_conditioned_v4a2":
        raise ValueError("metadata is not a v4-A2 temporal MLP artifact")

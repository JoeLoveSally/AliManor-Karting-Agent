#!/usr/bin/env python3
"""Train v4-A: per-frame shared CNN -> GRU -> state-conditioned KEEP/SWITCH."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Train v4-A with an explicit per-frame CNN + GRU time axis."
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help="v4-A intentionally reuses the exact v3 200ms sample manifest.",
    )
...
def target_horizons(raw: dict[str, object]) -> tuple[float, ...]:
    ...
    if not isinstance(values, list) or not values:
        raise ValueError("v4-A requires dataset.prediction_horizons_ms")
    ...
    with context:
        for batch_index, batch in enumerate(data_loader):
            ...
            if switch_logits.shape != switch_targets.shape:
                raise ValueError(
                    f"switch target/logit mismatch: {switch_targets.shape} vs "
                    f"{switch_logits.shape}"
                )
            if future_logits.shape != future_targets.shape:
                raise ValueError(
                    f"future target/logit mismatch: {future_targets.shape} vs "
                    f"{future_logits.shape}"
                )
            output_count = int(switch_logits.shape[1])
            if primary_output_index >= output_count:
                raise ValueError("primary_output_index exceeds horizon count")
...
def print_summary(
    ...
    print(
        ...
        f"negative_fpr={summary['primary_switch_false_positive_rate']:.3f}, "
...
def main() -> int:
    ...
    raw = load_raw_config(args.config.resolve())
    ...
    gru_hidden_dim = int(model_config.get("gru_hidden_dim", 128))
    ...
    print(
        f"V4-A: explicit_time_axis=True history={history_ms:g}ms "
        f"frame_stack={frame_stack} horizons={horizons} "
    ...
    try:
        ...
        metadata = {
            "model_family": "sequential_state_conditioned_v4a",

#!/usr/bin/env python3
"""Evaluate v3 static switch quality and stateful offline replay."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Evaluate v3 expert-conditioned switch classification and simulate the "
            "state-conditioned controller on held-out expert visuals."
        )
    )
    parser.add_argument(
        "--config", type=Path, default=ROOT / "configs" / "train_v3.yaml"
    )
    parser.add_argument(
        "--samples",
        type=Path,
...
def artifact_dir_from_config(raw: dict[str, object]) -> Path:
    ...
    return ROOT / "artifacts" / "models" / str(
        artifact.get("name", "mobilenet_v3_small_v3")
    )
...
def simulate_video(
    ...
    *,
    ...
    if initial is None:
        raise ValueError("v3 sample is missing current_pressed")
    state = bool(initial)
...
def main() -> int:
    ...
    output_path = (
        args.output.resolve()
        if args.output
        else artifact_dir / "evaluation" / f"{args.split}_v3.json"
    )
    ...
    if switch_if_release.shape != expected_shape or switch_if_press.shape != expected_shape:
        raise RuntimeError("v3 switch prediction shape mismatch")
